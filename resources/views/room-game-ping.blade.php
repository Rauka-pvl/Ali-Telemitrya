...
window.addEventListener('load', () => {
    ...
    const clientId = localStorage.getItem(`ping_client_id_${roomId}`) ?? `${Date.now()}-${Math.random().toString(16).slice(2)}`;
    localStorage.setItem(`ping_client_id_${roomId}`, clientId);
    let joined = false;
    ...
    const micLoop = () => {
        ...
        if (hz >= minHz && now - lastSentAt >= sendIntervalMs) {
            lastSentAt = now;
            post(`/room/${roomId}/movement`, { clientId, source:'ping', movement, hz, ts:now }).catch(() => {});
        }
